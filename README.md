...
- generate a scoop manifest file named `name.json` in the project directory that includes the download url and hash for the Windows x86_64 release
...
⚠️ Note: the finalized scoop manifest generated by `gopher release` does not currently get commited. This is a bit of a chicken and egg sitiation right now. Goreleaser needs to run first to generate the windows binary and checksum that then can be added to the scoop manifest. But for the gorleleaser to run all changes must be finalized and committed. Will fix this in a future release... Likely by pre-compiling the windows binary twice.
...
If a `dist` folder exists in the project directory and it contains a `checksums.txt` file (usually automatically generated by `goreleaser`) the hash for the Windows x86_64 binary will be injected into the manifest file right after the URL.
